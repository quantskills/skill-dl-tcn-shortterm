"""Holistic gate for validation-independent epoch trajectory averaging."""
...
class TrajectoryAverageDecision:
    status: str
    admitted: bool
    blockers: tuple[str, ...]
...
def decide_trajectory_average_seed7_gate(
    ...
    *,
    ...
    validation_volatility_ratio: float,
    raw_state_drift_max: float,
    arithmetic_mean_max_error: float,
    average_parameter_distance: float,
    average_update_count_min: int,
    average_update_count_max: int,
...
) -> TrajectoryAverageDecision:
    """Require broad task gain, V42 Pareto safety, and exact averaging mechanics."""
    ...
    try:
        ...
        raise ContractError("v44 comparisons are incomplete") from exc
    numeric = np.asarray(
        [
            *control.values(),
            *pointwise.values(),
            teacher_fidelity_delta,
            validation_volatility_ratio,
            raw_state_drift_max,
            arithmetic_mean_max_error,
            average_parameter_distance,
    ...
    if not np.isfinite(fold_values).all() or not np.isfinite(horizon_values).all():
        raise ContractError("v44 unit deltas must be finite")
    ...
    if len(rankic_bootstrap) != 1 or "bootstrap_ci_low" not in rankic_bootstrap:
        raise ContractError("v44 requires one RankIC bootstrap row")
    rankic_ci_low = float(cast(Any, rankic_bootstrap["bootstrap_ci_low"].iloc[0]))
    if not np.isfinite(rankic_ci_low):
        raise ContractError("v44 RankIC bootstrap bound must be finite")
    ...
    if validation_volatility_ratio > 0.90:
        blockers.append("validation_volatility_ratio_above_gate")
    if raw_state_drift_max != 0.0:
        blockers.append("raw_training_trajectory_drifted")
    if arithmetic_mean_max_error > 1e-6:
        blockers.append("arithmetic_mean_identity_failed")
    if average_parameter_distance <= 0.0:
        blockers.append("average_parameter_distance_not_positive")
    if average_update_count_min != 7 or average_update_count_max != 7:
        blockers.append("average_update_count_drifted")
    ...
    evidence: dict[str, float | int | bool] = {
        ...
        **{f"pointwise_{key}": value for key, value in pointwise.items()},
        ...
        "validation_volatility_ratio": validation_volatility_ratio,
        "raw_state_drift_max": raw_state_drift_max,
        "arithmetic_mean_max_error": arithmetic_mean_max_error,
        "average_parameter_distance": average_parameter_distance,
        "average_update_count_min": average_update_count_min,
        "average_update_count_max": average_update_count_max,
    ...
    return TrajectoryAverageDecision(
        status=(
            "trajectory_average_seed7_holistic_admitted_v44"
            if admitted
            else "stop_trajectory_average_seed7_v44"
        ),
        admitted=admitted,

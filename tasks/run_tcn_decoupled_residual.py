"""Run the immutable v16 seed-7 decoupled signed-residual TCN screen."""
...
def _validate_parent(config: dict[str, object]) -> dict[str, object]:
    ...
    if not receipt_path.is_file() or not selection_path.is_file():
        raise ContractError("v16 parent artifact is incomplete")
    ...
    if not isinstance(receipt_value, dict) or not isinstance(selection_value, dict):
        raise ContractError("v16 parent evidence must contain objects")
    ...
    if receipt.get("receipt_id") != expected_receipt_id:
        raise ContractError("v16 parent receipt identity drifted")
    if receipt.get("sealed_test_accessed") is not False:
        raise ContractError("v16 parent must not have accessed sealed test")
    if selection.get("status") != "stop_stabilized_residual_seed7_effect_v15":
        raise ContractError("v16 parent status drifted")
    outputs = receipt.get("outputs")
    if not isinstance(outputs, dict):
        raise ContractError("v16 parent receipt outputs are missing")
    for relative, expected_hash in outputs.items():
        path = parent / str(relative)
        if not path.is_file() or _sha256(path) != str(expected_hash):
            raise ContractError("v16 parent output hash drifted")
    return {
        "path": str(parent),
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run bounded seed-7 decoupled signed-residual TCN validation"
    )
    ...
    try:
        if output_dir.exists() or temporary.exists():
            raise ContractError("v16 decoupled screen refuses to overwrite artifacts")
        config_path = arguments.config.resolve()
        config_value = json.loads(config_path.read_text(encoding="utf-8"))
        if not isinstance(config_value, dict):
            raise ContractError("v16 decoupled config must contain an object")
        config = cast(dict[str, object], config_value)
        if _contains_secret_key(config):
            raise ContractError("v16 decoupled config contains a secret-like key")
        if config.get("precision") != "float32" or int(
            cast(Any, config["seed"])
        ) != 7:
            raise ContractError("v16 decoupled screen requires seed 7 float32")
        ...
        if missing := [name for name, path in source_paths.items() if not path.is_file()]:
            raise ContractError(f"v16 decoupled sources missing: {', '.join(missing)}")
        ...
        if not isinstance(expected_hashes, dict) or observed_hashes != {
            ...
            raise ContractError("v16 decoupled source SHA-256 identity drifted")
        ...
        if "sealed" not in raw_split or raw_split["sealed"].astype(bool).any():
            raise ContractError("v16 decoupled screen rejects sealed split rows")
        ...
        if unknown := sorted(observed_stages - allowed_stages):
            raise ContractError(
                "v16 decoupled split contains forbidden stages: "
                + ", ".join(unknown)
            )
        folds = tuple(
            int(cast(Any, value))
        ...
        if folds != tuple(range(5)):
            raise ContractError("v16 decoupled folds must be exactly 0 through 4")
        ...
        expected_candidates = (
            "decoupled-residual-c16-chomp-smooth-lr010",
            "decoupled-residual-c16-chomp-smooth-lr033",
        )
        if candidate_trial_ids != expected_candidates or {
            ...
            raise ContractError("v16 decoupled trial identities drifted")
        by_trial = {trial.trial_id: trial for trial in trials}
        if by_trial[control_trial_id].model_kind != "temporal_context":
            raise ContractError("v16 decoupled control kind drifted")
        residual_lrs = tuple(
            by_trial[trial_id].residual_learning_rate
            for trial_id in candidate_trial_ids
        )
        if residual_lrs != (0.0003, 0.001) or any(
            by_trial[trial_id].model_kind != "decoupled_temporal_context"
            or by_trial[trial_id].residual_scale != 0.05
            or by_trial[trial_id].adapter_learning_rate is not None
            for trial_id in candidate_trial_ids
        ):
            raise ContractError("v16 decoupled candidate mechanism drifted")
        ...
        effect = evaluate_decoupled_residual_seed7(
            tuning.leaderboard,
        ...
        receipt: dict[str, Any] = {
            "schema_version": "tcn-decoupled-signed-residual-v16/v1",

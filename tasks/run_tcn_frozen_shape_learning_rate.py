"""Run the immutable v29 frozen shape-only learning-rate experiment."""
...
def _paired_v28_comparison(
    current: pd.DataFrame, v28: pd.DataFrame
) -> pd.DataFrame:
    key = ["seed", "fold"]
    current_columns = key + [
        "best_epoch",
    ...
    current_values = current[current_columns].rename(
        columns={
            "best_epoch": "v29_best_epoch",
            "best_mean_daily_rankic": "v29_mean_daily_rankic",
            "dynamic_skip_shape_output_weight_l2": "v29_shape_output_weight_l2",
        }
    )
    v28_values = v28[v28_columns].rename(
        columns={
    ...
    paired = current_values.merge(
        v28_values, on=key, how="outer", validate="one_to_one", indicator=True
    )
    if set(paired["_merge"].astype(str)) != {"both"} or len(paired) != 15:
        raise ContractError("v29/v28 paired comparison coverage drifted")
    paired = paired.drop(columns="_merge")
    paired["v29_v28_rankic_delta"] = (
        paired["v29_mean_daily_rankic"] - paired["v28_mean_daily_rankic"]
    )
    paired["v29_v28_shape_output_weight_l2_delta"] = (
        paired["v29_shape_output_weight_l2"]
        - paired["v28_shape_output_weight_l2"]
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run the immutable v29 frozen shape learning-rate probe"
    )
    ...
    try:
        if output_dir.exists() or temporary.exists():
            raise ContractError("v29 refuses to overwrite experiment artifacts")
        config_path = arguments.config.resolve()
        config_value = json.loads(config_path.read_text(encoding="utf-8"))
        if not isinstance(config_value, dict):
            raise ContractError("v29 config must contain an object")
        config = cast(dict[str, object], config_value)
        if config.get("protocol_version") != "v29":
            raise ContractError("v29 protocol identity drifted")
        if _contains_secret_key(config):
            raise ContractError("v29 config contains a secret-like key")
        if config.get("precision") != "float32":
            raise ContractError("v29 precision must remain float32")
        ...
        if checkpoint_min_delta != 0.0 or patience_min_delta != 0.0005:
            raise ContractError("v29 decoupled selection thresholds drifted")
        seeds = tuple(
            int(cast(Any, value))
            for value in cast(list[object], config["seeds"])
        )
        if seeds != (7, 17, 27):
            raise ContractError("v29 seeds must be exactly 7, 17 and 27")
        if cast(list[object], config["folds"]) != [0, 1, 2, 3, 4]:
            raise ContractError("v29 folds must be exactly 0 through 4")

        expected_hashes_value = config.get("source_sha256")
        if not isinstance(expected_hashes_value, dict):
            raise ContractError("v29 source identities are missing")
        expected_hashes = {
        ...
        if missing := [name for name, path in source_paths.items() if not path.is_file()]:
            raise ContractError("v29 sources missing: " + ", ".join(missing))
        observed_hashes = {name: _sha256(path) for name, path in source_paths.items()}
        if observed_hashes != expected_hashes:
            raise ContractError("v29 source SHA-256 identity drifted")
        ...
        if len(trials) != 1 or trials[0].trial_id != candidate_trial_id:
            raise ContractError("v29 must train exactly one candidate")
        candidate = trials[0]
        if (
            ...
            or candidate.learning_rate != 0.001
            ...
            raise ContractError("v29 frozen shape-only contract drifted")
        ...
        if float(np.max(np.abs(diagnostic_scores - leaderboard_scores))) > 1e-12:
            raise ContractError("v29 diagnostic RankIC drifted from leaderboard")
        ...
        paired_v28 = _paired_v28_comparison(leaderboard, v28_rows)
        comparison = build_tcn_lstm_comparison(leaderboard, lstm)
        gates = cast(dict[str, object], config["gates"])
        decision = evaluate_frozen_shape_learning_rate_multiseed(
            leaderboard,
            historical,
            diagnostics,
            comparison,
        ...
        paired_v28.to_parquet(temporary / "v28-paired-comparison.parquet", index=False)
        ...
        receipt: dict[str, Any] = {
            "schema_version": "tcn-frozen-shape-learning-rate-v29/v1",

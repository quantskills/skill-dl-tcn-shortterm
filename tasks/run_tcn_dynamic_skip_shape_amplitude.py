"""Run the immutable v25 dynamic-skip shape/amplitude probe."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run the immutable v25 shape/amplitude probe"
    )
    ...
    try:
        if output_dir.exists() or temporary.exists():
            raise ContractError("v25 refuses to overwrite experiment artifacts")
        config_path = arguments.config.resolve()
        config_value = json.loads(config_path.read_text(encoding="utf-8"))
        if not isinstance(config_value, dict):
            raise ContractError("v25 config must contain an object")
        config = cast(dict[str, object], config_value)
        if config.get("protocol_version") != "v25":
            raise ContractError("v25 protocol identity drifted")
        if _contains_secret_key(config):
            raise ContractError("v25 config contains a secret-like key")
        if config.get("precision") != "float32":
            raise ContractError("v25 precision must remain float32")
        seeds = tuple(
            int(cast(Any, value))
            for value in cast(list[object], config["seeds"])
        )
        if seeds != (7, 17, 27):
            raise ContractError("v25 seeds must be exactly 7, 17 and 27")
        if cast(list[object], config["folds"]) != [0, 1, 2, 3, 4]:
            raise ContractError("v25 folds must be exactly 0 through 4")

        expected_hashes_value = config.get("source_sha256")
        if not isinstance(expected_hashes_value, dict):
            raise ContractError("v25 source identities are missing")
        expected_hashes = {
        ...
        if missing := [name for name, path in source_paths.items() if not path.is_file()]:
            raise ContractError("v25 sources missing: " + ", ".join(missing))
        observed_hashes = {name: _sha256(path) for name, path in source_paths.items()}
        if observed_hashes != expected_hashes:
            raise ContractError("v25 source SHA-256 identity drifted")
        ...
        if "sealed" not in raw_split or raw_split["sealed"].astype(bool).any():
            raise ContractError("v25 rejects sealed split rows")
        ...
        if unknown := sorted(observed_stages - {"train", "validation", "purged"}):
            raise ContractError(
                "v25 split contains forbidden stages: " + ", ".join(unknown)
            )
        ...
        if len(trials) != 1 or trials[0].trial_id != candidate_trial_id:
            raise ContractError("v25 must train exactly one candidate")
        candidate = trials[0]
        if (
            ...
            or candidate.dynamic_skip_token_normalization != "shape_log_rms"
            ...
            raise ContractError("v25 single-variable shape/amplitude contract drifted")
        ...
        if (
            ...
            raise ContractError("v25 LayerNorm ablation coverage drifted")
        ...
        decision = evaluate_dynamic_skip_shape_amplitude_multiseed(
            leaderboard,
            historical,
            diagnostics,
            comparison,
            ...
            min_amplitude_projection_weight_l2=float(
                cast(Any, gates["min_amplitude_projection_weight_l2"])
            ),
            ...
            historical_dynamic_parameter_count=int(
                cast(Any, gates["historical_dynamic_parameter_count"])
            ),
            ...
            amplitude_parameter_count=int(
                cast(Any, gates["amplitude_parameter_count"])
            ),
            scorer_input_width=int(cast(Any, gates["scorer_input_width"])),
        ...
        receipt: dict[str, Any] = {
            "schema_version": "tcn-dynamic-skip-shape-amplitude-v25/v1",

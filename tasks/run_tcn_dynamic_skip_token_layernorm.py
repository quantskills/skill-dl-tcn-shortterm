"""Run the immutable v24 dynamic-skip token LayerNorm probe."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run the immutable v24 token LayerNorm probe"
    )
    ...
    try:
        if output_dir.exists() or temporary.exists():
            raise ContractError("v24 refuses to overwrite experiment artifacts")
        config_path = arguments.config.resolve()
        config_value = json.loads(config_path.read_text(encoding="utf-8"))
        if not isinstance(config_value, dict):
            raise ContractError("v24 config must contain an object")
        config = cast(dict[str, object], config_value)
        if config.get("protocol_version") != "v24":
            raise ContractError("v24 protocol identity drifted")
        if _contains_secret_key(config):
            raise ContractError("v24 config contains a secret-like key")
        if config.get("precision") != "float32":
            raise ContractError("v24 precision must remain float32")
        seeds = tuple(
            int(cast(Any, value))
            for value in cast(list[object], config["seeds"])
        )
        if seeds != (7, 17, 27):
            raise ContractError("v24 seeds must be exactly 7, 17 and 27")
        if cast(list[object], config["folds"]) != [0, 1, 2, 3, 4]:
            raise ContractError("v24 folds must be exactly 0 through 4")

        expected_hashes_value = config.get("source_sha256")
        if not isinstance(expected_hashes_value, dict):
            raise ContractError("v24 source identities are missing")
        expected_hashes = {
        ...
        if missing := [name for name, path in source_paths.items() if not path.is_file()]:
            raise ContractError("v24 sources missing: " + ", ".join(missing))
        observed_hashes = {name: _sha256(path) for name, path in source_paths.items()}
        if observed_hashes != expected_hashes:
            raise ContractError("v24 source SHA-256 identity drifted")
        ...
        if "sealed" not in raw_split or raw_split["sealed"].astype(bool).any():
            raise ContractError("v24 rejects sealed split rows")
        ...
        if unknown := sorted(observed_stages - {"train", "validation", "purged"}):
            raise ContractError(
                "v24 split contains forbidden stages: " + ", ".join(unknown)
            )
        ...
        if len(trials) != 1 or trials[0].trial_id != candidate_trial_id:
            raise ContractError("v24 must train exactly one candidate")
        candidate = trials[0]
        if (
            ...
            or candidate.dynamic_skip_token_normalization != "layer_norm"
            ...
            raise ContractError("v24 single-variable LayerNorm contract drifted")
        ...
        decision = evaluate_dynamic_skip_token_layernorm_multiseed(
            leaderboard,
            historical,
            diagnostics,
            parent_diagnostics,
            ...
            max_parent_variation_cv_ratio=float(
                cast(Any, gates["max_parent_variation_cv_ratio"])
            ),
        ...
        receipt: dict[str, Any] = {
            "schema_version": "tcn-dynamic-skip-token-layernorm-v24/v1",

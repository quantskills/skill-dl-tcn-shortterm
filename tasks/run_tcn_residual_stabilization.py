"""Run the immutable v15 seed-7 stabilized signed-residual TCN screen."""
...
def _validate_parent(config: dict[str, object]) -> dict[str, object]:
    ...
    if not receipt_path.is_file() or not selection_path.is_file():
        raise ContractError("v15 parent artifact is incomplete")
    ...
    if not isinstance(receipt_value, dict) or not isinstance(selection_value, dict):
        raise ContractError("v15 parent evidence must contain objects")
    ...
    if receipt.get("receipt_id") != expected_receipt_id:
        raise ContractError("v15 parent receipt identity drifted")
    if receipt.get("sealed_test_accessed") is not False:
        raise ContractError("v15 parent must not have accessed sealed test")
    if selection.get("status") != "stop_signed_candidate_unstable_v14":
        raise ContractError("v15 parent status drifted")
    outputs = receipt.get("outputs")
    if not isinstance(outputs, dict):
        raise ContractError("v15 parent receipt outputs are missing")
    for relative, expected_hash in outputs.items():
        path = parent / str(relative)
        if not path.is_file() or _sha256(path) != str(expected_hash):
            raise ContractError("v15 parent output hash drifted")
    return {
        "path": str(parent),
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run bounded seed-7 stabilized signed-residual TCN validation"
    )
    ...
    try:
        if output_dir.exists() or temporary.exists():
            raise ContractError("v15 stabilization refuses to overwrite artifacts")
        config_path = arguments.config.resolve()
        config_value = json.loads(config_path.read_text(encoding="utf-8"))
        if not isinstance(config_value, dict):
            raise ContractError("v15 stabilization config must contain an object")
        config = cast(dict[str, object], config_value)
        if _contains_secret_key(config):
            raise ContractError("v15 stabilization config contains a secret-like key")
        if config.get("precision") != "float32" or int(
            cast(Any, config["seed"])
        ) != 7:
            raise ContractError("v15 stabilization requires seed 7 float32")
        ...
        if missing := [name for name, path in source_paths.items() if not path.is_file()]:
            raise ContractError(f"v15 stabilization sources missing: {', '.join(missing)}")
        ...
        if not isinstance(expected_hashes, dict) or observed_hashes != {
            ...
            raise ContractError("v15 stabilization source SHA-256 identity drifted")
        ...
        if "sealed" not in raw_split or raw_split["sealed"].astype(bool).any():
            raise ContractError("v15 stabilization rejects sealed split rows")
        ...
        if unknown := sorted(observed_stages - allowed_stages):
            raise ContractError(
                "v15 stabilization split contains forbidden stages: "
                + ", ".join(unknown)
            )
        folds = tuple(
            int(cast(Any, value))
        ...
        if folds != tuple(range(5)):
            raise ContractError("v15 stabilization folds must be exactly 0 through 4")
        ...
        candidate_trial_ids = tuple(
            str(value)
            for value in cast(list[object], config["candidate_trial_ids"])
        )
        expected_candidates = (
            "stable-residual-c16-chomp-smooth-lr100",
            "stable-residual-c16-chomp-smooth-lr010",
        )
        if candidate_trial_ids != expected_candidates or {
            trial.trial_id for trial in trials
        } != {control_trial_id, *candidate_trial_ids} or len(trials) != 3:
            raise ContractError("v15 stabilization trial identities drifted")
        by_trial = {trial.trial_id: trial for trial in trials}
        if by_trial[control_trial_id].model_kind != "temporal_context":
            raise ContractError("v15 stabilization control kind drifted")
        candidate_lrs = tuple(
            by_trial[trial_id].adapter_learning_rate
            for trial_id in candidate_trial_ids
        )
        if candidate_lrs != (0.003, 0.0003) or any(
            by_trial[trial_id].model_kind != "stabilized_temporal_context"
            or by_trial[trial_id].residual_scale != 0.05
            for trial_id in candidate_trial_ids
        ):
            raise ContractError("v15 stabilization candidate mechanism drifted")
        ...
        effect = evaluate_stabilized_residual_seed7(
            tuning.leaderboard,
            ...
            required_parameter_count=int(
                cast(Any, gates["required_parameter_count"])
            ),
        )
        if effect.winner_trial_id is not None:
            comparison_trial_id = effect.winner_trial_id
        else:
            candidates = effect.summary.loc[
                effect.summary["trial_id"].isin(candidate_trial_ids)
            ].sort_values(
                ["mean_rankic", "worst_fold_rankic", "trial_id"],
                ascending=[False, False, True],
                kind="mergesort",
            )
            comparison_trial_id = str(candidates.iloc[0]["trial_id"])
        ...
        comparison_output: dict[str, object] = {
            **comparison,
            "tcn_trial_id": comparison_trial_id,
        }
        ...
        selection = {
            ...
            "confirmation_seeds_authorized": list(
                final.confirmation_seeds_authorized
            ),
        ...
        receipt: dict[str, Any] = {
            "schema_version": "tcn-stabilized-signed-residual-v15/v1",
